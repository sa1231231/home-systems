...
export function buildMerge(rows: SheetRow[], cluster: Cluster): MergeOp {
  ...
  const allCols = new Set<string>([
    ...Object.keys(canonicalRow.record),
    ...dupRows.flatMap((r) => Object.keys(r.record)),
  ]);

  for (const col of allCols) {
    const canonValue = canonicalRow.record[col] ?? "";
    if (col === LEGACY_NOTES) {
      // Concat all non-empty legacy_notes from canonical + dups, separated.
      const pieces: string[] = [];
      if (canonValue) pieces.push(canonValue);
      for (const dup of dupRows) {
        const v = dup.record[LEGACY_NOTES] ?? "";
        if (v && !pieces.includes(v)) pieces.push(v);
      }
      const combined = pieces.join("\n---\n");
      if (combined !== canonValue) fills[col] = combined;
      continue;
    }
    if (canonValue !== "") continue; // never overwrite a non-empty canonical cell
    for (const dup of dupRows) {
      const v = dup.record[col] ?? "";
      if (v !== "") {
        fills[col] = v;
        break;
      }
